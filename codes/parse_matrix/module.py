...
hm_chrs = ['chr1','chr2','chr3','chr4','chr5','chr6','chr7','chr8','chr9','chr10','chr11','chr12','chr13','chr14','chr15','chr16','chr16','chr17','chr18','chr19','chr20','chr21','chr22','chrX','chrY']
...
class gene():
	def __init__(self, chr, source, geneid, start, end, sense, genetype, transcripts, symb, elseinfo):
		self.__chr = chr
...
#	def len(self): return abs(self.__end - self.__start + 1)
	...
	def transcripts(self): return self.__transcripts
	def symb(self): return self.__symb
	def elseinfo(self): return self.__elseinfo
	def setTranscripts(self, transcripts): self.__transcripts = transcripts
	def addTranscripts(self, transcripts): self.__transcripts.append(transcripts)
	def overlaps(self, otherGene): #sense overlap
		if self.chr() != otherGene.chr(): return False
		elif not(self.sense() == '.' or \
			otherGene.sense() == '.' or \
			self.sense() == otherGene.sense()): return False
		elif self.start() > otherGene.end() or otherGene.start() > self.end(): return False
		else: return True
	def overlaps2(self, otherGene): #overlap
		if self.chr() != otherGene.chr(): return False
		elif self.start() > otherGene.end() or otherGene.start() > self.end(): return False
		else: return True
	def overlaps3(self, otherGene):	#antisense overlap
		if self.chr() != otherGene.chr(): return False
		elif self.sense() == otherGene.sense(): return False
		elif self.start() > otherGene.end() or otherGene.start() > self.end(): return False
		else: return True
	def addElseinfo(self, elseinfo): self.__elseinfo.update(elseinfo)
	def setElseinfo(self, elseinfo): self.__elseinfo = elseinfo
	def setGeneid(self, geneid): self.__geneid = geneid
	def setSymbol(self, symb): self.__symb = symb


class transcript():
	def __init__(self, chr, source, geneid, trxid, start, end, exons, sense, genetype, trxtype, symb, endsup, elseinfo):
		self.__chr = chr
		self.__source = source
		self.__geneid = geneid
		self.__trxid = trxid
		...
		for n in range(len(exons)):
			if n < len(exons) - 1: self.__introns.append(exon(chr, exons[n].trxid(), int(exons[n].end()) + 1, int(exons[n+1].start() - 1), sense, exons[n].symb(), exons[n].endsup(),exons[n].elseinfo()))
	def chr(self): return self.__chr
	def source(self): return self.__source
	def geneid(self): return self.__geneid
...
#	def len(self): return abs(self.__end - self.__start + 1)
	...
	def symb(self): return self.__symb
	def endsup(self): return self.__endsup
	def elseinfo(self): return self.__elseinfo
	...
	def setElseinfo(self, elseinfo): self.__elseinfo = elseinfo
	def addElseinfo(self, elseinfo): self.__elseinfo.update(elseinfo)
	def setEndsup(self, endsup): self.__endsup = endsup
	...
	def addExons(self, exons): self.__exons.append(exons)
...
class exon():
	def __init__(self, chr, trxid, start, end, sense, symb, endsup, elseinfo):
		self.__chr = chr
		self.__trxid = trxid
		self.__start = start
		self.__end = end
...
def checkEndSup(trxid):
	if 'TSS' in trxid and 'CPS' not in trxid:
		endsup = 'TSS'
	elif 'CPS' in trxid and 'TSS' not in trxid:
		endsup = 'CPS'
	elif 'TSS' in trxid and 'CPS' in trxid:
		endsup = 'TSS,CPS'
	else:
		endsup = ''
	return endsup
...
def defineEndsup(trx):
	if 'TSS' in trx.trxid() and 'CPS' in trx.trxid():
		trx.addAttri('TSS,CPS')
	elif 'TSS' in trx.trxid() and not 'CPS' in trx.trxid():
		trx.addAttri('TSS')
	elif not 'TSS' in trx.trxid() and 'CPS' in trx.trxid():
		trx.addAttri('CPS')
...
def overlapTrxs(trxs, prefix, chr):
	newTrxs = []
	overT = []
...
#		print len(ptrxs)
		otherTrxs = [x for j,x in enumerate(trxs) if j != i]
		overlap = [trx]
		for j in xrange(len(otherTrxs)):
			ot = otherTrxs[j]
			if not trx.overlaps(ot): continue
			else: overlap.append(ot)
		overT.append(overlap)
	mergeT = mergeOverlaps(overT)
	x = 0
	for t in xrange(len(mergeT)):
		x += 1
		trxs = mergeT[t]
		y = 0
		iden = []
		for z in xrange(len(trxs)):
#			if str(trx) == 'chr1(+):77965588-77970651': print x
#			if str(trx) == 'chr1(-):110880026-110881359': print x
			trx = trxs[z]
			if trx in iden: continue
			otrxs = [sub for j,sub in enumerate(trxs) if j != z]
			for otrx in otrxs:
				osample = otrx.trxid().split('-chr')[0]
				if identical(trx, otrx):
#					trx.addAttri(osample)
					iden.append(otrx)
#					print str(trx)
#					print trx.attri()
			y += 1
			newGeneid = prefix+'-'+chr+'-'+str(x)
#			if newGeneid == 'BIG-imm-chr1-21':
#				print x
#				print [str(test) for test in trxs]
			newTrxid = newGeneid + '.' + str(y)
			trx.setGeneid(newGeneid)
			trx.setTrxid(newTrxid)
			newTrxs.append(trx)
	return newTrxs

def defineGenetype(alltrxs):
	genedic = dict()
	for t in alltrxs:
		geneid = t.geneid()
		if not genedic.has_key(geneid):
			genedic[geneid] = []
		genedic[geneid].append(t)
	for geneid in genedic.keys():
		trxs = genedic[geneid]
		trxtypes = map(lambda x: x.trxtype(), trxs)
		if 'intervening' in trxtypes:
			for trx in trxs:
				trx.setGenetype('intervening')
		elif 'antisense' in trxtypes:
			for trx in trxs:
				trx.setGenetype('antisense')
		elif any("flanking" in s for s in trxtypes):
			for trx in trxs:
				trx.setGenetype('flanking')
		else:
			for trx in trxs:
				trx.setGenetype('lincRNA')
	return alltrxs
...
def writeMatrix(matdic, samples, filename):
...
#	samples = matdic.values()[0].keys()
	...
	for idsym in matdic.keys():
		...
		for sample in samples:
			outfile.write('\t'+str(express[sample]))
		outfile.write('\n')	
	outfile.close()

def writeinfoMatrix(matdic, samples, filename):
	outfile = open(filename, 'w')
...
def getGtf2(filename):
	...
	lines = filein.readlines(); filein.close()
	gtfChr = dict()
	known_info = ['gene_id', 'transcript_id', 'gene_type', 'transcript_type', 'gene_name', 'endsup']
	preGeneid =''
...
#			if not line[2] == 'gene': continue
			...
			source = line[1]; feature = line[2]
			geneid = re.search('.*gene_id "(\S+)";', temp).group(1)
			if feature != 'gene':
				trxid = re.search('.*transcript_id "(\S+)";', temp).group(1)
			start = line[3]; end = line[4]; sense = line[6]
			infos = [y.strip() for y in [x.split('"')[0] for x in temp.strip().split(';')] if y]
			unknowninfo = list(set(infos) - set(known_info))
...
#			preChr = chrom; preStart = int(line[3].split(' ')[0]); preEnd = int(line[4].split(' ')[0]); preSense = line[6].split(' ')[0]
			...
			if temp.find('gene_name') > -1:
				...
				symb = ""
			if temp.find('endsup') > -1:
				endsup = temp.split('endsup')[1].split(';')[0].split('"')[1]
			else:
				endsup = ''
			if len(unknowninfo) != 0:
				elseinfo = dict()
				for ninfo in unknowninfo:
					info = [s for s in [x.strip() for x in temp.strip().split(';')] if ninfo in s][0]
					if not '"' in info: 
						key = info; value = ''
					else:
						key = info.split('"')[0].strip(); value = info.split('"')[1].strip()
					elseinfo[key] = value
#				elseinfo = ';'.join(elseinfo)+';'
			else: elseinfo = {}
			...
			if feature == 'gene':
				preGeneid = geneid
				transcripts = []
				ngene = gene(chrom, source, geneid, int(start), int(end), sense, genetype, transcripts, symb, elseinfo)
				gtfChr[chrom].append(ngene)
			elif feature == 'transcript':
				preTrxid = trxid
				exons = []
				ntranscript = transcript(chrom, source, geneid, trxid, int(start), int(end), exons, sense, genetype, trxtype, symb, endsup, elseinfo)
				if ntranscript.geneid() == preGeneid: 
					ngene.addTranscripts(ntranscript)
				
			elif feature == 'exon':
				nexon = exon(chrom, trxid, int(start), int(end), sense, symb, endsup, elseinfo)
				if nexon.trxid() == preTrxid: ntranscript.addExons(nexon)
	return gtfChr
...
def getGtf(filename):
	...
	gtfChr = dict(); exons = []
	known_info = ['gene_id', 'transcript_id', 'gene_type', 'transcript_type', 'gene_name', 'endsup']
	preChr = ''; preGeneID = ''; preTrxID = ''; preStart = -1; preEnd = -1; preSense = ''; preGenetype = ''; preTrxtype = ''; preSymb = ''; preEndsup = ''; preElseinfo = ''
...
#				elseinfo = ';'.join(elseinfo)+';'
			...
			if i > 0 and line[2] == 'transcript':
				#print line[2]
				exons.sort(cmp0)
				ntranscript = transcript(preChr, source, preGeneID, preTrxID, preStart, preEnd, exons, preSense, preGenetype, preTrxtype, preSymb, endsup, elseinfo)
				if preChr != '': gtfChr[preChr].append(ntranscript)
				exons = []
			elif line[2] == 'exon':
				#print line[2]
				nexon = exon(chrom, trxid, int(line[3]), int(line[4]), line[6], symb, endsup, elseinfo); exons.append(nexon)
			preGeneID = geneid; preTrxID = trxid; preGenetype = genetype; preTrxtype = trxtype; preSymb = symb; preEndsup = endsup; preElseinfo = elseinfo
			
			if line[2] == 'transcript': 
				preChr = chrom; preStart = int(line[3].split(' ')[0]); preEnd = int(line[4].split(' ')[0]); preSense = line[6].split(' ')[0]; preEndsup = endsup
			if len(lines) - 2 == i:
				exons.sort(cmp0)
				ntranscript = transcript(preChr,source, preGeneID, preTrxID, preStart, preEnd, exons, preSense, preGenetype, preTrxtype, preSymb, endsup, elseinfo)
				if preChr != '': gtfChr[preChr].append(ntranscript)
...
def sortChrs(chrs):
	...
	for chr in chrs:
		n = chr.split('chr')[-1]
		if n != 'X' and n != 'Y' and n!= 'M': numChrs.append(int(n))
		else: strChrs.append(n)
	sort = sorted(numChrs) + sorted(strChrs)
	sortedChrs = map(lambda x: 'chr'+str(x), sort)
	return sortedChrs
...
#	fileout = open(outfilename, 'a')
	...
	for chr in gtfD.keys():
		...
		for trx in gtf:
			elseinfo = ''
			keys = sorted(trx.elseinfo().keys())
			for key in keys:
				value = trx.elseinfo()[key]
				elseinfo += key+' "'+value+'";'
#			line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\";' % (chr, 'CorrectedCuff', 'transcript', trx.start(), trx.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid())	
			line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\"; gene_name \"%s\"; gene_type \"%s\"; transcript_type \"%s\"; endsup \"%s\"; %s' % (chr, 'CAFE', 'transcript', trx.start(), trx.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid(), trx.symb(), trx.genetype(), trx.trxtype(), trx.endsup(), elseinfo)
			lines.append(line)
			exons = trx.exons()
			for ex in exons:
#				line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\";' % (chr, 'CorrectedCuff', 'exon', ex.start(), ex.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid())
				line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\"; gene_name \"%s\"; gene_type \"%s\"; transcript_type \"%s\"; endsup \"%s\"; %s' % (chr, 'CAFE', 'exon', ex.start(), ex.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid(), trx.symb(), trx.genetype(), trx.trxtype(), trx.endsup(), elseinfo)
				lines.append(line)
	fileout.write('\n'.join(lines))
	fileout.write('\n')
	fileout.close()
...
#	for geneid in geneDic.keys():
#		trxs = geneDic[geneid]
#		geneattri = ','.join([x for x in list(set(sum(map(lambda x: x.attri().split(','), trxs),[]))) if x])
#		symbol = trxs[0].symb()
#		if trxs[0].symb() == '': symbol = trxs[0].geneid()
...
#		geneline = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; gene_type \"%s\"; gene_name \"%s\"; attribute \"%s\";' % (trxs[0].chr(), 'BIG', 'gene', geneStart, geneEnd, '.', trxs[0].sense(), '.', trxs[0].geneid(), trxs[0].genetype(), symbol, geneattri)
#		lines.append(geneline)
#		for trx in trxs:
#			line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\"; gene_name \"%s\"; gene_type \"%s\"; transcript_type \"%s\"; endsup \"%s\"; attribute \"%s\";' % (trx.chr(), 'BIG', 'transcript', trx.start(), trx.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid(), symbol, trx.genetype(), trx.trxtype(), trx.endsup(), trx.attri())
#			lines.append(line)
#			exons = trx.exons()
#			for ex in exons:
##				line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\";' % (chr, 'CorrectedCuff', 'exon', ex.start(), ex.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid())
#				line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\"; gene_name \"%s\"; gene_type \"%s\"; transcript_type \"%s\"; attribute \"%s\";' % (trx.chr(), 'BIG', 'exon', ex.start(), ex.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid(), symbol, trx.genetype(), trx.trxtype(), trx.attri())
#				lines.append(line)
#	fileout.write('\n'.join(lines))
#	fileout.write('\n')
#	fileout.close()

def writeGtf3(gtfD, outfilename):
	fileout = open(outfilename, 'w')
	lines = []
	geneDic = dict()
	chrs = sortChrs(gtfD.keys())
	for chr in chrs:
		genes = gtfD[chr]
		genes.sort(key=lambda x: (x.start(), x.end()))
		for gene in genes:
			elseinfo = ''
			keys = sorted(gene.elseinfo().keys())
			for key in keys:
				value = gene.elseinfo()[key]
				elseinfo += key+' "'+value+'"; '
			elseinfo.strip()
#		endsup = attributes[0][0]; typeNum = len(list(set(sum(map(lambda x: x[-1].split('types')[-1].split(','), attributes), []))))
#		geneattri = endsup+'|'+str(typeNum)+ 'types'
			line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; gene_type \"%s\"; gene_name \"%s\"; %s' % (gene.chr(), gene.source(), 'gene', gene.start(), gene.end(), '.', gene.sense(), '.', gene.geneid(), gene.genetype(), gene.symb(), elseinfo)
			lines.append(line)
			...
			for trx in trxs:
				...
				for key in keys:
					...
					elseinfo += key+' "'+value+'"; '
				elseinfo.strip()
				line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\"; gene_name \"%s\"; gene_type \"%s\"; transcript_type \"%s\"; endsup \"%s\"; %s' % (trx.chr(), gene.source(), 'transcript', trx.start(), trx.end(), '.', trx.sense(), '.', gene.geneid(), trx.trxid(), gene.symb(), trx.genetype(), trx.trxtype(), trx.endsup(), elseinfo)
				lines.append(line)
				exons = trx.exons()
				for ex in exons:
					elseinfo = ''
					keys = sorted(ex.elseinfo().keys())
					for key in keys:
						value = ex.elseinfo()[key]
						elseinfo += key+' "'+value+'"; '
					elseinfo.strip()
#				line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\";' % (chr, 'CorrectedCuff', 'exon', ex.start(), ex.end(), '.', trx.sense(), '.', trx.geneid(), trx.trxid())
					line = '%s\t%s\t%s\t%i\t%i\t%s\t%s\t%s\tgene_id \"%s\"; transcript_id \"%s\"; gene_name \"%s\"; gene_type \"%s\"; transcript_type \"%s\"; %s' % (trx.chr(), gene.source(), 'exon', ex.start(), ex.end(), '.', trx.sense(), '.', gene.geneid(), trx.trxid(), gene.symb(), gene.genetype(), trx.trxtype(), elseinfo)
					lines.append(line)
	fileout.write('\n'.join(lines))
	fileout.write('\n')
	fileout.close()
