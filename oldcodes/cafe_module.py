...
class transcript():
	def __init__(self, chr, geneid, trxid, start, end, exons, sense, genetype):
		self.__chr = chr
...
def getGtf(filename):
	...
	#lines = lines.split('\n'); filein.close()
	gtfChr = dict(); exons = []
	preChr = ''; preGeneID = ''; preTrxID = ''; preStart = -1; preEnd = -1; preSense = ''; preGenetype = ''
...
#			trxid = line[-1].split(';')[1].split('"')[1]
			...
			if i > 0 and line[2] == 'transcript':
				#print line[2]
				exons.sort(cmp0)
				ntranscript = transcript(preChr, preGeneID, preTrxID, preStart, preEnd, exons, preSense, preGenetype)
				if preChr != '': gtfChr[preChr].append(ntranscript)
				exons = []
			elif line[2] == 'exon':
				#print line[2]
				nexon = exon(chrom, int(line[3]), int(line[4]), line[6]); exons.append(nexon)
			preGeneID = geneid; preTrxID = trxid; preGenetype = genetype
...
def getSeq(locus, f):
	...
	commandbase = 'nibFrag ' + f + '/' + chr + '.nib ' + str(start) + ' ' + str(end) + ' ' + str(sense) + ' stdout'
	p=Popen(commandbase, shell=True, stdout=PIPE, stdin=PIPE, stderr=PIPE)
